  ...
  while (queue.length > 0) {
    ...
    if (ps.openCount === num && ps.closeCount === num) {
      ...
      if (ps.openCount < num) { // if we can add an open parentheses, add it
        queue.push(new ParenthesesString(ps.str.concat("("), ps.openCount + 1, ps.closeCount));
      }
      if (ps.openCount > ps.closeCount) { // if we can add a close parentheses, add it
        queue.push(new ParenthesesString(ps.str.concat(")"), ps.openCount, ps.closeCount + 1));
      }
    }
  }
  return result;
...
console.log(`All combinations of balanced parentheses are: ${generate_valid_parentheses2(3)}`);